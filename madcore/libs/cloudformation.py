...
class StackManagement(PluginsBase):
        ...
        def show_output(results_key, column_names):
            ...
            if results_key in stack_details:
                for param in stack_details[results_key]:
                    data.append([param.get(c, '') for c in
                                 column_names])

                self.show_table_output(column_names, data)

        self.logger.info('[%s] Output parameters for stack:',
                         stack_details['StackName'])
        show_output('Outputs', ['OutputKey', 'OutputValue',
                                'Description'])

    def stack_show_input_parameter(
            self,
            stack_name,
            input_params,
            debug=True,
        ...
        if input_params != [{}]:
            if debug:
                self.logger.info('[%s] Input parameters for stack:',
                                 stack_name)
            show_output(['ParameterKey', 'ParameterValue'])
        else:
            if debug:
                self.logger.info('[%s] No input parameters for stack.',
                                 stack_name)
    ...
    def wait_for_stack_to_complete(self, stack_name):
        self._cf_client.get_waiter('stack_create_complete'
    ...
    def delete_stack_if_exists(self, stack_name):
        ...
        if stack_details is not None:
            ...
            self.logger.info('[%s] Stack does not exists, skip.',
                             stack_name)
    ...
    def create_stack(
        ...
        response = self.cf_client.create_stack(StackName=stack_name,
                                               TemplateBody=stack_template_body,
                                               Parameters=input_parameters, Capabilities=capabilities
                                                                                         or [])
    ...
    def create_stack_if_not_exists(
        ...
        stack_params = \
            self.create_stack_parameters(dict_params=dict_params)
        ...
        if stack_details is None:
            self.logger.info('[%s] Stack does not exists, creating it...'
                             , stack_name)
            self.create_stack(stack_name, stack_template_body,
                              stack_params, capabilities=capabilities)
        elif self.is_stack_create_failed(stack_details):
            self.logger.info("[%s] Stack is created but failed with status '%s'"
                             , stack_details['StackName'],
            ...
            self.logger.info('[%s] Stack create in progress, wait to finish...'
                             , stack_name)
            self.show_create_stack_events_progress(stack_name)
            self.logger.info('[%s] Stack finished.', stack_name)
        else:
            self.logger.info('[%s] Stack already exists, skip.',
                             stack_name)
            updated = self.update_stack_if_changed(stack_name,
                                                   stack_template_body, stack_details, dict_params,
                                                   capabilities)
            exists = True
        ...
        if stack_details \
                and not self.is_stack_create_failed(stack_details):
            self.logger.info("[%s] Stack created with status '%s'.\n",
                             stack_details['StackName'],
                             stack_details['StackStatus'])
        else:
            self.logger.error('[%s] Error while creating stack. Check logs for details.'
                              , stack_name)
            error = True
    ...
    def update_stack(
        ...
        response = self.cf_client.update_stack(**stack_params)
    ...
    def update_stack_if_changed(
        ...
        self.logger.info('[%s] Try to update stack if needed.',
                         stack_name)
        ...
        for stack_param in stack_details.get('Parameters', []):
            param = {'ParameterKey': stack_param['ParameterKey'],
                     'UsePreviousValue': True}
            if stack_param['ParameterKey'] in stack_update_parameters:
                if str(stack_param['ParameterValue']) \
                        != str(stack_update_parameters[stack_param['ParameterKey'
                        ]]):
        ...
        if updated_params:
            self.logger.info('[%s] Stack params changed, show params that require update.'
                             , stack_name)
            self.stack_show_input_parameter(stack_name, updated_params,
                                            debug=False)
            self.logger.info('[%s] Start updating stack.', stack_name)
            self.update_stack(stack_name, stack_template_body,
                              stack_update_params, capabilities,
                              show_progress)
            updated = True
        else:
            self.logger.info('[%s] There are no params to update, skip.'
                             , stack_name)
    ...
    def create_stack_parameters(cls, dict_params=None):
        ...
        if not dict_params:
            ...
            for (param_key, param_value) in dict_params.items():
                cf_param = {'ParameterKey': param_key,
    ...
    def start_instance_if_not_running(self, instance_id, log_label=''):
        self.logger.info('%sCheck if madcore instance is running...',
                         log_label)
        try:
            ...
            if instance_status in ['stopped', 'stopping']:
                self.logger.info("%sMadcore instance is not running, current status is: '%s'."
                                 , log_label, instance_status)
                self.logger.info('%sStart madcore instance...',
                                 log_label)
                ...
                ec2_cli.get_waiter('instance_running'
                                   ).wait(InstanceIds=[instance_id])
                self.logger.info('%sMadcore instance is running.',
                                 log_label)
                return True
            else:
                self.logger.info('%sMadcore instance is already running.'
                                 , log_label)
        except botocore.exceptions.ClientError, ec2_error:
            self.logger.error("%sError while starting instance '%s'.",
                              log_label, instance_id)
    ...
    def stop_instance_if_running(self, instance_id, log_label=''):
        ...
        try:
            ...
            if instance_status in ['pending', 'running', 'stopping']:
                self.logger.info("%sMadcore instance is running, current status is: '%s'."
                                 , log_label, instance_status)
                self.logger.info('%sStop madcore instance...',
                                 log_label)
                ec2_cli = self.get_aws_client('ec2')
                ...
                ec2_cli.get_waiter('instance_stopped'
                                   ).wait(InstanceIds=[instance_id])
                self.logger.info('%sMadcore instance is stopped.',
                                 log_label)
                return True
            else:
                self.logger.info('%sMadcore instance is already stopped.'
                                 , log_label)
        except botocore.exceptions.ClientError, ec2_error:
            self.logger.error("%sError while stopping instance '%s'.",
                              log_label, instance_id)
            self.logger.error(ec2_error)
...
class StackCreate(StackManagement, Command):
    ...
    def take_action(self, parsed_args):
        ...
        (s3_stack, s3_exists, _) = \
            self.create_stack_if_not_exists(const.STACK_S3,
                                            s3_stack_template, {})
        s3_bucket_name = self.get_output_from_dict(s3_stack['Outputs'],
                                                   'S3BucketName')
        ...
        network_parameters = OrderedDict((('Type', self.env.title()),
                                          ('Env', env), ('VPCCIDRBlock', '10.99.0.0/16')))
        network_stack_template = \
            self.get_cf_template_local('network.json')
        (network_stack, network_exists, _) = \
            self.create_stack_if_not_exists(const.STACK_NETWORK,
                                            network_stack_template, network_parameters)

        self.log_figlet('STACK %s', const.STACK_FOLLOWME)

        # create SGFM

        sgfm_parameters = OrderedDict((('FollowMeIpAddress',
                                        self.get_ipv4()), ('VpcId',
                                                           self.get_output_from_dict(network_stack['Outputs'],
                                                                                     'VpcId'))))
        sgfm_stack_template = self.get_cf_template_local('sgfm.json')
        (sgfm_stack, sgfm_exists, _) = \
            self.create_stack_if_not_exists(const.STACK_FOLLOWME,
                                            sgfm_stack_template, sgfm_parameters)
        ...
        core_repo_config = self.get_repo_info('core',
                                              include_remote=False)
        plugins_repo_config = self.get_repo_info('plugins',
                                                 include_remote=False)

        core_parameters = OrderedDict((
            ('FollowmeSecurityGroup',
             self.get_output_from_dict(sgfm_stack['Outputs'],
                                       'FollowmeSgId')),
            ('PublicNetZoneA',
             self.get_output_from_dict(network_stack['Outputs'],
                                       'PublicNetZoneA')),
        ...
        if core_stack is not None:
            if self.is_stack_create_in_progress(core_stack):
                ...
                self.logger.info('[%s] Stack finished.',
                                 const.STACK_CORE)
            elif not self.is_stack_create_failed(core_stack):
                core_instance_id = \
                    self.get_output_from_dict(core_stack['Outputs'],
                                              'MadCoreInstanceId')
                self.logger.debug('[%s] Check if madcore instance is terminated...'
                                  , const.STACK_CORE)
                if self.is_instance_terminated(core_instance_id):
                    self.logger.info('[%s] Madcore instance is terminated, recreate stack.'
                                     , const.STACK_CORE)
                    self.logger.info('[%s] Delete stack.',
                                     core_stack['StackName'])
                    self.delete_stack(const.STACK_CORE)
                else:
                    self.logger.debug('[%s] Instance not terminated.',
                                      const.STACK_CORE)
        ...
        (core_stack, core_exists, _) = \
            self.create_stack_if_not_exists(const.STACK_CORE,
                                            core_stack_template, core_parameters,
                                            capabilities=core_capabilities)

        core_public_ip = self.get_output_from_dict(core_stack['Outputs'
                                                   ], 'MadCorePublicIp')
        ...
        dns_parameters = OrderedDict((('DomainName',
                                       user_config['domain']), ('SubDomainName',
                                                                user_config['sub_domain']), ('EC2PublicIP',
                                                                                             core_public_ip)))
        dns_stack_template = self.get_cf_template_local('dns.json')
        (dns_stack, dns_exists, dns_updated) = \
            self.create_stack_if_not_exists(const.STACK_DNS,
                                            dns_stack_template, dns_parameters)
        ...
        name_servers = \
            self.get_hosted_zone_name_servers(self.get_output_from_dict(dns_stack['Outputs'
                                                                        ], 'HostedZoneID'))
        ...
        if delegation_response.get('verified', False):
            ...
            self.logger.error('DNS delegation error: %s',
                              delegation_response)
        ...
        self.logger.info("Wait until DNS for domain '%s' is resolved..."
                         , domain_name)
        ...
        while True:
            domain_ip = \
                utils.hostname_resolves(config.get_full_domain(),
                                        max_time=timeouts.DNS_RESOLVE_TIMEOUT)

            if domain_ip != core_public_ip:
                self.logger.warn("Domain '%s' points to '%s' but should point to '%s'"
                                 , config.get_full_domain(), domain_ip,
                                 core_public_ip)
